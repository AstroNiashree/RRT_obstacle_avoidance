...
title('RRT Algorithm UAV planning path');
...
while failedAttempts <= maxFailedAttempts  % loop to grow RRTs
    ...
    distanceCost(RRT(:,1:3),sample)
    [A, I] = min( distanceCost(RRT(:,1:3),sample) ,[],1) % find the minimum value of each column of A
    AL = [A I] % for debugging
    closestNode = RRT(I(1),1:3)
    ...
    [A, I2] = min( distanceCost(RRT(:,1:3),newPoint) ,[],1); % check if new node is not already pre-existing in the tree
    if distanceCost(newPoint,RRT(I2(1),1:3)) < threshold, failedAttempts = failedAttempts + 1; continue; end 
    
   
    RRT = [RRT; newPoint I(1)]; % add node
